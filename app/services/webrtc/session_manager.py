...
class WebRTCSession:
    ...
    # Buffer for ICE candidates generated by server PC (to be fetched by client for trickle ICE)
    outgoing_ice_candidates: list[dict[str, str | int | None]] = field(
        default_factory=list
    )
...
class WebRTCSessionManager:
    ...
    def get_outgoing_ice_candidates(
        self, session_id: str, clear: bool = False
    ) -> list[dict[str, str | int | None]]:
        """Return server-generated ICE candidates for client trickle polling (clear optional, default no)."""
        session = self._sessions.get(session_id)
        if session is None:
            raise KeyError(f"Unknown WebRTC session: {session_id}")
        cands = list(session.outgoing_ice_candidates)
        if clear:
            session.outgoing_ice_candidates.clear()
        return cands
    ...
    def _ensure_tracks_on_pc(self, session: WebRTCSession) -> None:
        ...
        # Reset pacing on resume/re-attach (even for already-attached tracks on re-neg offer).
    ...
    async def _recreate_peer_connection(self, session: WebRTCSession) -> None:
        ...
        Clears pending/outgoing candidates (will re-arrive or be resent by client for new PC).
        """
    ...
    def _register_peer_handlers(self, session: WebRTCSession) -> None:
        ...
        def on_icecandidate(event) -> None:
            """Capture server-generated ICE candidates and buffer for client to poll/fetch (trickle ICE support)."""
            if event.candidate:
                cand_dict: dict[str, str | int | None] = {
                ...
                session.outgoing_ice_candidates.append(cand_dict)
                logger.debug(
                    "Buffered server ICE candidate for session %s (mid=%s)",
                    session_id,
                    cand_dict.get("sdp_mid"),
                )
            # event.candidate is None signals end-of-candidates (can be ignored for trickle polling)
